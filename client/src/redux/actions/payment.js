...
export const get_payment_total = (shipping_id, coupon_name) => async dispatch => {
    ...
    try {
        const res = await axios.get(
            `${import.meta.env.VITE_API_URL}/api/payment/get-payment-total?shipping_id=${shipping_id}&coupon_name=${coupon_name}`,
            config
        );
        ...
        if (res.status === 200 && !res.data.error) {
            ...
            dispatch({ type: GET_PAYMENT_TOTAL_FAIL });
        }
    } catch (err) {
        dispatch({ type: GET_PAYMENT_TOTAL_FAIL });
    }
...
export const get_client_token = () => async dispatch => {
    ...
    try {
        const res = await axios.get(
            `${import.meta.env.VITE_API_URL}/api/payment/get-token`,
            config
        );
        ...
        if (res.status === 200) {
            ...
            dispatch({ type: LOAD_BT_TOKEN_FAIL });
        }
    } catch (err) {
        dispatch({ type: LOAD_BT_TOKEN_FAIL });
    }
...
export const process_payment = (
    ...
    try {
        const res = await axios.post(
            `${import.meta.env.VITE_API_URL}/api/payment/make-payment`,
            body,
            config
        );
