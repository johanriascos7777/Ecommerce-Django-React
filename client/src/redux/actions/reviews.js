...
export const get_reviews = product_id => async dispatch => {
    ...
    try {
        const res = await axios.get(
            `${import.meta.env.VITE_API_URL}/api/reviews/get-reviews/${product_id}`,
            config
        );
        if (res.status === 200) {
...
export const get_review = product_id => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.get(
                `${import.meta.env.VITE_API_URL}/api/reviews/get-review/${product_id}`,
                config
            );
            if (res.status === 200) {
    ...
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post(
                `${import.meta.env.VITE_API_URL}/api/reviews/create-review/${product_id}`,
                body,
                config
            );
            if (res.status === 201) {
    ...
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.put(
                `${import.meta.env.VITE_API_URL}/api/reviews/update-review/${product_id}`,
                body,
                config
            );
            if (res.status === 200) {
    ...
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.delete(
                `${import.meta.env.VITE_API_URL}/api/reviews/delete-review/${product_id}`,
                config
            );
            if (res.status === 200) {
                dispatch({ type: DELETE_REVIEW_SUCCESS, payload: res.data });
            } else {
                dispatch({ type: DELETE_REVIEW_FAIL }); // ✅ era 'tye' — corregido
            }
        } catch(err) {
            dispatch({ type: DELETE_REVIEW_FAIL }); // ✅ era 'tye' — corregido
        }
...
export const filter_reviews = (product_id, rating) => async dispatch => {
    ...
    let myRating;
    if (rating === 0.5)       myRating = '0.5';
    else if (rating === 1.0)  myRating = '1.0';
    else if (rating === 1.5)  myRating = '1.5';
    else if (rating === 2.0)  myRating = '2.0';
    else if (rating === 2.5)  myRating = '2.5';
    else if (rating === 3.0)  myRating = '3.0';
    else if (rating === 3.5)  myRating = '3.5';
    else if (rating === 4.0)  myRating = '4.0';
    else if (rating === 4.5)  myRating = '4.5';
    else                      myRating = '5.0';
    ...
    try {
        ...
        if (res.status === 200) {
            dispatch({ type: FILTER_REVIEWS_SUCCESS, payload: res.data });
        } else {
            dispatch({ type: FILTER_REVIEWS_FAIL });
        }
    } catch(err) {
        dispatch({ type: FILTER_REVIEWS_FAIL });
    }
